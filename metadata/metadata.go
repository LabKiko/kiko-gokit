...
// AppendToServerContext returns a new context with the provided kv merged
// with any existing metadata in the context.
func AppendToServerContext(ctx context.Context, kv ...string) context.Context {
	if len(kv)%2 == 1 {
		panic(fmt.Sprintf("metadata: AppendToServerContext got an odd number of input pairs for metadata: %d", len(kv)))
	}
	md, _ := FromServerContext(ctx)
	md = md.Clone()
	for i := 0; i < len(kv); i += 2 {
		md.Set(kv[i], kv[i+1])
	}
	return NewServerContext(ctx, md)
}

// MergeToServerContext merge new metadata into ctx.
func MergeToServerContext(ctx context.Context, cmd Metadata) context.Context {
	md, _ := FromServerContext(ctx)
	md = md.Clone()
	for k, v := range cmd {
		md[k] = v
